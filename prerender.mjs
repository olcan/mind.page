...
// on the page itself. usage: node prerender.mjs [url]  (default https://mindbox.io; a localhost
...
const url = process.argv[2] ?? 'https://mindbox.io'
if (new URL(url).hostname.match(/^(localhost|127\.0\.0\.1)$/)) process.env.FIRESTORE_EMULATOR_HOST ??= '127.0.0.1:8080'
...
console.log(
  `captured frozen render of ${url}: ${(html.length / 1024).toFixed(0)}KB html, stored in prerender/anonymous`
)
process.exit(0)
