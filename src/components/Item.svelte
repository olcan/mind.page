<script lang="ts">
  ...
  function toHTML(
    ...
    // Candidate BYTES never enter the transforms; a surviving marker is classified at
    // these final Marked seams (an enclosing _removed transform may deliberately drop
    // its marker along with its span of text). No pre-parse fence prediction; the
    // decoded bytes never enter Markdown.
