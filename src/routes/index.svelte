<script lang="ts">
  ...
  // clear *_output & remove *_log blocks before a run (candidate-safe, reviews 149 §2,
  // 151 §3): stale artifacts are re-generated by the run, and the block primitives run
  // over the grammar view so a candidate's nested _output/_log openers survive exactly
  function clearRunArtifacts(text: string): string {
    return editInertText(text, g => removeBlock(clearBlock(g, '\\w*?_output'), '\\w*?_log'))
  }
  ...
  if (isClient) {
    ...
    // now-vault-routed item. computed from the scanner's grammar view, never resolved
    // item state.
    window['_vault_routed'] = (text: string) => isVaultRouted(text)
    // the safe read-modify-write seam for index-sensitive whole-item rewrites (bridge
    // design §2.3, review 148 §2): the transform runs over the grammar view and the raw
    // vault_result envelopes are restored, so stored-item programs never persist opaque
    // markers or shift indices on a candidate's fake content
    window['_vault_edit'] = (
      text: string,
      transform: (grammarText: string) => string,
      options?: { allowDrop?: boolean }
    ) => editInertText(text, transform, options)
    // the ONE versioned grammar capability (reviews 178 §5.1, 179): an atomically
    // assigned object, so a consumer can never observe a new version with old
    // functions. version 2 = the inert-region grammar. new stored consumers REQUIRE
  ...
  class _Item {
    ...
    // undefined after side effects) is never called at all. a PROTOTYPE getter, not an
    // instance field: the enumerable own-property surface (see the constructor) is the
    // documented json(_this) shape and must not change
    get write_accepts(): boolean {
      return true
    }
    get name(): string {
    ...
    read(type: string = '', options: object = {}) {
      ...
      // copy item GRAMMAR view (for possible post-processing below): the read path
      // feeds macro evaluation and typed block extraction, which must never see raw
      // candidate bytes (design §2.3); transient replacements are scanned on the fly
      // replace item text if a replacement is specified in replace_items
      const replacement = options['replace_items']?.[item.name]
      let text =
        replacement != null ? scanInert(replacement).grammarText : (item.vaultScan?.grammarText ?? item.text)
      ...
      if (options['eval_macros']) {
        // the read path caches its macro expansion normally (review 149 §3): it stores
        // the GRAMMAR/marker-domain text, which is inert; the collision the round-148 fix
        // targeted is the RENDERER publishing placeholder HTML into the SAME item.expanded
        // -- so only the renderer bypasses candidate expanded state (see Item.svelte).
        // making the read transient too created a permanent ~250ms idle re-expansion loop.
        const ignore_expanded = ignore_expanded_state
        // note the reset conditions here (deephash, version, etc) should match those in toHTML in Item.svelte
        if (
          !ignore_expanded &&
          item.expanded &&
          ...
          if (!ignore_expanded) item.expanded = {} // reset macro expansion state
          let cacheIndex = 0
          const replaceMacro = (m, js) => {
            if (!isBalanced(js)) return m // skip unbalanced macros that are probably not macros, e.g. ((x << 2) >> 2)
            try {
          ...
          if (!ignore_expanded) {
            ...
            // eval macros without updating expanded state (transient)
    ...
    write(text: string, type: string = '_output', options = {}): boolean {
      ...
      if (type.trim()) {
        // typed-block writes run the block primitives over the GRAMMAR VIEW and restore
        // the raw vault_result envelopes (review 149 §2): a malformed candidate can
        // contain a nested _log|_output opener whose bare fence a raw removeBlock could
        // strip, unclaiming the rest of the candidate on the next scan. capture the raw
        // text once; the local `out` reproduces the original this.text-getter chaining.
        __item.text = editInertText(this.text, grammar => {
          if (type.endsWith('_log')) {
            // remove existing *_log blocks; only write the new one if non-empty
            let out = removeBlock(grammar, '\\w*?_log')
            if (text) out = appendBlock(out, type, text)
            return out
          }
          // non-log block: append it, then move any existing _log to the bottom
          let out = appendBlock(grammar, type, text)
          const _log = extractBlock(out, '_log')
          if (_log) {
            out = removeBlock(out, '_log')
            out = appendBlock(out, '_log', _log)
          }
          return out
        })
      } else {
        // replace whole item (callers pass the exact intended text)
        __item.text = text
      }
    ...
    clear(type: string, options) {
      ...
      // candidate-safe (review 149 §2): clearBlock over the grammar view, raw envelopes restored
      this.write(editInertText(this.text, grammar => clearBlock(grammar, type)), '', options)
    }

    remove(type: string, options) {
      if (!type) throw new Error('remove(type) requires block type')
      this.write(editInertText(this.text, grammar => removeBlock(grammar, type)), '', options)
    }
  ...
  function updateItemLayout() {
    ...
    const minColumnWidth = 750 // minimum column width for multiple columns (columns grow to .column max-width)
  ...
  function itemTextChanged(
    ...
    item.hash = hash(text) // identity stays on RAW text (design §2.3: editing,
    ...
    item.header = item.vaultScan.grammarText
      .match(/^(?:\s*<.*>\s*)*([^\n]*)/)
      .pop()
  ...
  function appendJSOutput(index: number) {
    ...
    let jsin = extractBlock(item.vaultScan?.grammarText ?? item.text, 'js_input')
    ...
    if (!(jsout instanceof Promise)) {
      ...
      // candidate-safe block append (review 149 §2): a malformed candidate can hold a
      // nested _output opener that a raw appendBlock context could disturb
      if (jsout !== undefined) item.text = editInertText(item.text, g => appendBlock(g, '_output', jsout))
    }
    ...
    if (log) {
      item.text = editInertText(item.text, g => appendBlock(g, '_log', log))
  ...
  function onItemEditing(
    ...
    if (editing) {
      ...
      if (run && !cancelled && item.runnable /* just updated */) {
        item.text = clearRunArtifacts(item.text)
  ...
  function onItemRun(index: number = -1, e: MouseEvent = null, touch_first = true) {
    ...
    if (item.attr?.source) {
      ...
      // SOURCE-SIDE selection via the edit seam (reviews 148 §1.2, 150 §2.3): the match
      // runs over the grammar view -- so a candidate-nested js_input is never selected --
      // and restoration carries a candidate INSIDE a selected input into the child as its
      // exact raw envelope (the child's own scanner masks it), never as a literal marker.
      // suffix normalization (hide inputs via _removed, without doubling an existing
      // suffix) also happens on the grammar view, so it cannot rename a nested opener
      // inside a restored envelope. candidates outside the selected inputs are dropped.
      let inputs_text: string | null = null
      try {
        inputs_text = editInertText(
          item.text,
          grammar => {
            const inputs = grammar.match(input_regex)
            if (!inputs) return '' // no inputs: drops everything; refused below
            return inputs
              .join('\n')
              .trim()
              .replace(input_regex, m => m.replace(/_input(?:_hidden|_removed)?(?= |$)/im, '_input_removed'))
          },
          { allowDrop: true }
        )
      } catch {
        inputs_text = null // an unclaimable candidate split: refuse rather than leak
      }
      if (!inputs_text) {
  ...
  async function fetchPreview(item) {
    ...
    try {
      ...
      if (item.previewable) {
        ...
        if (
          _.isEqual(
            extractBlock(item.vaultScan?.grammarText ?? item.text, '\\S*').split('\n').filter(requires_manual_preview),
  ...
  function installStyleItem(item) {
    ...
    const css = extractBlock(item.vaultScan?.grammarText ?? item.text, 'css_style')
  ...
  function initItems() {
    ...
    items.forEach(item => {
      ...
      const init_block_type = extractBlock(item.vaultScan?.grammarText ?? item.text, 'js_init') ? 'js_init' : 'js'
  ...
  async function initialize(): Promise<boolean> {
    ...
    for (const item of items) {
      ...
      // this runs BEFORE the initial itemTextChanged pass, so item.vaultScan is not yet
      // set -- scan INLINE (review 148 §1.1: a raw fallback here would let a malformed
      // candidate's nested js block execute during a reload)
      const js = extractBlock(scanInert(item.text).grammarText, 'js')
      if (!js) continue
      try {
...
<style>
  ...
  .column {
    ...
    min-width: 0px;
    max-width: 1000px; /* see minColumnWidth (750) for the effective min-width in multi-column layouts */
