...
<script lang="ts">
	...
	function updateItemIndices() {
		...
		if (document.getElementById("editor")) {
			let columnCount = Math.round(
				window.innerWidth /
					document.getElementById("editor").clientWidth
			);
	...
	function onEditorChange(origText: string) {
		...
		items.forEach((item) => {
			...
			item.pinned = lctext.match(/(?:^|\s)#pin(?:\/|\s|$)/)
				? true
				: false;
			...
			if (item.prefixMatch) {
				item.prefixMatchTerm =
					terms[0] +
					lctext.substring(terms[0].length).match(/^[\/\w]*/)[0];
			}
	...
	function onEditorDone(text: string, e: KeyboardEvent = null) {
		...
		onEditorChange(editorText);
		if (editing)
			setTimeout(() => textArea(indexFromId.get(tmpid)).focus(), 0);
	...
	function focusOnNearestEditingItem(index: number) {
		...
		if (near == Infinity)
			near = Math.max.apply(null, [-1, ...editingItems]);
		focusedItem = near;
		textArea(near).focus();
		...
		firebase()
			...
			.onAuthStateChanged((authUser) => {
				if (authUser) {
					...
					user.getIdToken(false /*force refresh*/)
						.then((token) => {
							document.cookie =
								"__session=" + token + ";max-age=86400";
		...
		window["_google"] = function () {
			window.open(
				"https://google.com/search?q=" +
					encodeURIComponent(editorText.trim())
			);
		};
