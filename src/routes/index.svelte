<script lang="ts">
  ...
  function clearRunArtifacts(text: string): string {
    return editVaultText(text, g => removeBlock(clearBlock(g, '\\w*?_output'), '\\w*?_log'))
  }
  ...
  if (isClient) {
    ...
    ) => editVaultText(text, transform, options)
  ...
  class _Item {
    ...
    read(type: string = '', options: object = {}) {
      ...
      let text =
        replacement != null ? scanVaultResults(replacement).grammarText : (item.vaultScan?.grammarText ?? item.text)
    ...
    write(text: string, type: string = '_output', options = {}): boolean {
      ...
      if (type.trim()) {
        ...
        __item.text = editVaultText(this.text, grammar => {
    ...
    clear(type: string, options) {
      ...
      this.write(editVaultText(this.text, grammar => clearBlock(grammar, type)), '', options)
    }

    remove(type: string, options) {
      if (!type) throw new Error('remove(type) requires block type')
      this.write(editVaultText(this.text, grammar => removeBlock(grammar, type)), '', options)
    }
  ...
  function itemTextChanged(
    ...
    item.vaultScan = scanVaultResults(text)
  ...
  function appendJSOutput(index: number) {
    ...
    if (!(jsout instanceof Promise)) {
      ...
      if (jsout !== undefined) item.text = editVaultText(item.text, g => appendBlock(g, '_output', jsout))
    }
    ...
    if (log) {
      item.text = editVaultText(item.text, g => appendBlock(g, '_log', log))
  ...
  import { editVaultText, isVaultRouted, scanVaultResults } from '../vault_result'
  ...
  async function initialize(): Promise<boolean> {
    ...
    for (const item of items) {
      ...
      const js = extractBlock(scanVaultResults(item.text).grammarText, 'js')
      if (!js) continue
      try {
