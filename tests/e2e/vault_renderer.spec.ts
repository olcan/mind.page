...
test('the renderer reads real hidden stores, saves nothing, and follows store-only changes', async ({ page }) => {
  ...
  const view = await page.evaluate(async name => {
    ...
    return {
      ...
      comments: [...content.querySelectorAll('.vault-source .vault-comment')].map(c => [c.tagName.toLowerCase(), c.textContent, getComputedStyle(c).fontFamily, getComputedStyle(c).color, getComputedStyle(c).fontSize, getComputedStyle(c).backgroundColor, getComputedStyle(c).padding]),
      // an inline-code element of the body (the first one that is not a comment): the comment inherits the app's code typography
      inlineCode: (c => (c ? [getComputedStyle(c).fontFamily, getComputedStyle(c).fontSize] : null))(content.querySelector('.vault-source p > code:not(.vault-jinja):not(.vault-comment)') as HTMLElement | null),
  ...
  expect(view.comments.length, 'one html comment element').toBe(1)
  expect(view.comments[0].slice(0, 2), 'the trailing html comment is a code element with its exact text').toEqual(['code', '<!-- trailing note -->'])
  expect([view.comments[0][2], view.comments[0][4]], 'the comment inherits the app\'s code font stack and size from the code rule').toEqual(view.inlineCode)
  expect(view.comments[0][2].toLowerCase(), 'the app\'s code font stack').toContain('jetbrains mono')
  expect([view.comments[0][3], view.comments[0][5], view.comments[0][6]], 'gray, with the code background and padding neutralized').toEqual(['rgb(106, 115, 125)', 'rgba(0, 0, 0, 0)', '0px'])
